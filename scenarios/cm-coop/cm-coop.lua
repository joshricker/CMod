...
function cm_generate_standardized_ores()
  if not global.map_config.spawn_standardized_ore_patches then return end
  ...
  function cmgenstatingore(cmname, cmsizex, cmsizey)
    if cmname == "crude-oil" or cmname == "angels-natural-gas" then
      for y=1,cmsizey do
        surface.create_entity({name=cmname, amount=global.map_config.standardized_ore_oil_amount, position={-radius+cmxoreoffset+1+origin_spawn.x+cmoilx, -radius+cmyoreoffset+origin_spawn.y}})
        cmoilx=cmoilx+cmsizex
      end
      cmoilx = 0
      cmyoreoffset = cmsizey+cmyoreoffset+global.map_config.standardized_ore_gap
    else
      for x=0,cmsizex do
        for y=0,cmsizey do
          surface.create_entity({name=cmname, amount=global.map_config.standardized_ore_amount, position={-radius+cmxoreoffset+origin_spawn.x+x, -radius+cmyoreoffset+origin_spawn.y+y}})
        end
      end
      cmyoreoffset = cmsizey+cmyoreoffset+global.map_config.standardized_ore_gap
    end
  end
  cmgenstatingore("coal", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y)
  if game.item_prototypes["angels-ore1"] then
    if global.map_config.standardized_ore_double_iron then 
      cmgenstatingore("angels-ore1", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y*2)
    else
      cmgenstatingore("angels-ore1", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y)
    end
    cmgenstatingore("angels-ore3", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y)
    cmgenstatingore("angels-ore6", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y)
    cmgenstatingore("angels-ore5", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y)
    cmgenstatingore("crude-oil", global.map_config.standardized_ore_oil_gap, global.map_config.standardized_ore_oil_count)
  ...
  elseif game.item_prototypes["y-res1"] then
  	if global.map_config.standardized_ore_double_iron then 
      cmgenstatingore("iron-ore", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y*2)
    else
      cmgenstatingore("iron-ore", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y)
    end
    cmgenstatingore("copper-ore", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y)
    cmgenstatingore("y-res1", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y)
    cmgenstatingore("y-res2", global.map_config.standardized_ore_x, global.map_config.standardized_ore_y)
    cmgenstatingore("crude-oil", global.map_config.standardized_ore_oil_gap, global.map_config.standardized_ore_oil_count)
